...
def main():
    ...
    unofficial_leaves: list[tuple[str, list[int]]] = [
        (DR_SAUMYA_SHUKLA, list(range(1, 28 + 1))),
        (DR_RASHMI_SHARMA, list(range(1, 28 + 1))),
        (DR_ASHISH_GUPTA, [1, 6, 7, 23, 24]),
        (DR_AMIT_TRIPATHI, [8, 9, 11, 12]),
        (DR_HITESH_KUREEL, [5, 6, 7]),
        (DR_MINAKSHI_MISHRA, [26, 27, 28]),
    ]
    ...
    for dt in dates:
        ...
        if week == weeks.index("Sun"):
            ...
            #     sunday_doctor_rotation_order
            # )
            fixed_shifts[(DR_DEEPARK_SINGH, day)] = ["night"]
    ...
    first_night_off = DR_VIKAS_VERMA
