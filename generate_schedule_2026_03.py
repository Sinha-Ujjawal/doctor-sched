...
def main():
    ...
    unofficial_leaves: list[tuple[str, list[int]]] = [
        (DR_KRITIKA_PRASAD, [4, 5, 7, 8]),
        (DR_VIKAS_VERMA, [3, 5]),
        (DR_MINAKSHI_MISHRA, [8]),
        (DR_SAUMYA_SHUKLA, list(range(1, 32))),
        (DR_RASHMI_SHARMA, list(range(1, 32))),
    ]
    ...
    # DR_KRITIKA_PRASAD's Fixed shifts
    fixed_shifts[(DR_KRITIKA_PRASAD, 3)] = ["night"]
    fixed_shifts[(DR_KRITIKA_PRASAD, 6)] = ["morning"]

    # 4th is Holi day
    fixed_shifts[(DR_HITESH_KUREEL, 4)] = ["morning"]
    fixed_shifts[(DR_RAJAT_GUPTA, 4)] = ["evening"]
    fixed_shifts[(DR_MINAKSHI_MISHRA, 4)] = ["night"]

    # Sunday duties are fixed
    day = 1
    fixed_shifts[(DR_AKASHDEEP_GUPTA, day)] = ["morning", "evening"]
    fixed_shifts[(DR_RAJAT_GUPTA, day)] = ["night"]
    fixed_shifts[(DR_AMIT_TRIPATHI, day)] = ["ot_duty"]
    day = 8
    fixed_shifts[(DR_ASHISH_GUPTA, day)] = ["morning", "evening"]
    fixed_shifts[(DR_AKASHDEEP_GUPTA, day)] = ["night"]
    fixed_shifts[(DR_ABHILASHA_MISHRA, day)] = ["ot_duty"]
    day = 15
    fixed_shifts[(DR_VIKAS_VERMA, day)] = ["morning", "evening"]
    fixed_shifts[(DR_ASHISH_GUPTA, day)] = ["night"]
    fixed_shifts[(DR_ASHOK_KUMAR, day)] = ["ot_duty"]
    day = 22
    fixed_shifts[(DR_HITESH_KUREEL, day)] = ["morning", "evening"]
    fixed_shifts[(DR_VIKAS_VERMA, day)] = ["night"]
    fixed_shifts[(DR_AMIT_TRIPATHI, day)] = ["ot_duty"]
    day = 29
    ...
    if solution_maybe is not None:
        ...
        with ExcelWriter(excel_output, engine="openpyxl") as xlw:  # type: ignore
            ...
            def _populate_unavailable_leaves():
                ...
                pd.DataFrame(
                    unofficial_leaves_rows, columns=["Doctor", "Unavailability"]
                ).to_excel(
