...
def main():
    ...
    month = 7
    """
    DR_AMIT_TRIPATHI on leave on 2nd July 2025
    DR_MINAKSHI_MISHRA on leave on 11, 12, 13 and 14
    DR_RASHMI_SHARMA night duty on 4, 11, 18 and 25
    DR_RASHMI_SHARMA will do Sunday's Morning duty on 13th
    Dr. Hitesh Kureel is a new doctor EMO doctor
    DR_SAUMYA_SHUKLA will do ot duty on 2nd July
    """
    all_doctors = [
        DR_MINAKSHI_MISHRA,
        DR_ABHILASHA_MISHRA,
    ...
    leaves = [
        (DR_AMIT_TRIPATHI, [2]),
        (DR_MINAKSHI_MISHRA, [11, 12, 13, 14]),
    ]
    for dt in dates:
        day = dt.day
        ...
        if day == 1:
            unavailable_shifts[(DR_KRITIKA_PRASAD, day)] = all_shifts
    ...
    for dt in dates:
        day = dt.day
        if 1 <= day <= 15:
            unavailable_shifts[(DR_RASHMI_SHARMA, day)].append("morning")
