...
def main():
    ...
    unofficial_leaves: list[tuple[str, list[int]]] = [
        (DR_MINAKSHI_MISHRA, [28, 29, 30]),
        (DR_KRITIKA_PRASAD, [1, 2, 3, 4]),
    ]
    ...
    for dt in dates:
        ...
        if week == weeks.index("Sun"):
            # Fixed OT Duty in order
            if day < 31:
                doc = sunday_ot_duty_rotation_order[sunday_ot_duty_rotation_start]
                if day > 0:
                    fixed_shifts[(doc, day - 1)] = ["ot_duty"]  # Same doc on Sat.
                fixed_shifts[(doc, day)] = ["ot_duty"]
                sunday_ot_duty_rotation_start = (
                    sunday_ot_duty_rotation_start + 1
                ) % len(sunday_ot_duty_rotation_order)
            ...
            fixed_shifts[(doc, day)] = ["morning", "evening"]
            sunday_morning_evening_duty_rotation_start = (
        ...
        # NOTE: Wed OT will be on rotation
    ...
    # Dr. Abhilasha will do 30th (Sat) OT Duty so that load on Amit is less
    fixed_shifts[(DR_ABHILASHA_MISHRA, 30)] = ["ot_duty"]
    # Dr. Amit will do 31st (Sun) OT Duty following the rotation
    fixed_shifts[(DR_AMIT_TRIPATHI, 31)] = ["ot_duty"]
    ...
    minmax_ot_duty_shifts = {
        # DR_ASHOK_KUMAR: (6, 8),
        # DR_AMIT_TRIPATHI: (8, 11),
        # DR_ABHILASHA_MISHRA: (6, 8),
        # DR_SAUMYA_SHUKLA: (6, 8),
    }
