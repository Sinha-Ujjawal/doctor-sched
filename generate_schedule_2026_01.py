...
def main():
    ...
    unofficial_leaves = [
        [DR_KRITIKA_PRASAD, range(1, 16 + 1)],
        [DR_ABHILASHA_MISHRA, range(14, 18 + 1)],
        [DR_RAJAT_GUPTA, [1]],
        [DR_MINAKSHI_MISHRA, range(1, 31 + 1)],
    ]
    ...
    for dt in dates:
        ...
        if week in (weeks.index("Mon"),):
            fixed_shifts[(DR_ASHOK_KUMAR, day)] = ["ot_duty", "night"]
        if week in (weeks.index("Tue"),):
            fixed_shifts[(DR_AMIT_TRIPATHI, day)] = ["ot_duty"]
        if week in (weeks.index("Thu"),) and not (14 <= day <= 18):
    ...
    minmax_night_shifts = {
        # EMOs
        DR_SUNITA_OJHA: (0, 0),
        DR_KRITIKA_PRASAD: (2, 2),
        DR_MINAKSHI_MISHRA: (0, 0),
    ...
    first_night_off = DR_HITESH_KUREEL
