...
def main():
    ...
    unavailable_shifts = {}
    leaves = [
        (DR_RASHMI_SHARMA   , [17, 18, 19, 20, 21]),
        (DR_KRITIKA_PRASAD  , [3, 4, 5, 6]),
        (DR_MINAKSHI_MISHRA , [28, 29, 30]),
        (DR_MADHURI_TRIPATHI, [21, 22, 23, 24, 25, 26]),
        (DR_AMIT_TRIPATHI   , [21, 22, 23, 24, 25, 26]),
    ]
    for dt in dates:
        day = dt.day
    ...
    for dt in dates:
        ...
        if day == 4:
            fixed_shifts[(DR_MADHURI_TRIPATHI, day)] = ["morning", "evening"]
        if day == 11:
    ...
    def my_custom_constraints(model, shift_vars):
        ...
        for dt in dates:
            ...
            if week in (weeks.index("Thu"), weeks.index("Sun")):
                if week == weeks.index("Thu") and day in (1, 8, 15, 22):
