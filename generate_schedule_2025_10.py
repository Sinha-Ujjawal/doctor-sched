...
from doctor_schedule import (all_shifts, generate_month_dates,
                             generate_schedule, weeks)
from doctors import (DR_ABHILASHA_MISHRA, DR_AMIT_TRIPATHI, DR_ASHOK_KUMAR,
                     DR_HITESH_KUREEL, DR_KRITIKA_PRASAD, DR_MADHURI_TRIPATHI,
                     DR_MINAKSHI_MISHRA, DR_RASHMI_SHARMA, DR_SAUMYA_SHUKLA)
...
def main():
    ...
    leaves = [
        [DR_AMIT_TRIPATHI, [16, 17]],
        [DR_MADHURI_TRIPATHI, list(range(16, 26 + 1))],
        [DR_MINAKSHI_MISHRA, [21, 26, 27, 28, 29]],
        [DR_KRITIKA_PRASAD, [23, 24, 25, 26]],
        [DR_SAUMYA_SHUKLA, [22, 28]],
    ]
    # Unavailable shifts
    for dt in dates:
        day = dt.day
    ...
    # Unofficial leaves:
    #   DR_RASHMI_SHARMA will not be available on 20, 21 and 22
    unavailable_shifts[(DR_RASHMI_SHARMA, 20)] = all_shifts
    unavailable_shifts[(DR_RASHMI_SHARMA, 21)] = all_shifts
    unavailable_shifts[(DR_RASHMI_SHARMA, 22)] = all_shifts
    ...
    for dt in dates:
        ...
        if week in (weeks.index("Thu"),):
            ...
            fixed_shifts[(DR_RASHMI_SHARMA, day)] = ["morning"]
        if week in (weeks.index("Fri"),):
            fixed_shifts[(DR_SAUMYA_SHUKLA, day)] = ["ot_duty"]
        if week != weeks.index("Sun"):
            fixed_shifts[(DR_MADHURI_TRIPATHI, day)] = ["evening"]
    fixed_shifts[(DR_KRITIKA_PRASAD, 15)] = ["morning"]
    fixed_shifts[(DR_RASHMI_SHARMA, 5)] = ["morning", "evening"]
    # 21st is Diwali, and DR_AMIT_TRIPATHI agreed to do both ot_duty and night
    fixed_shifts[(DR_AMIT_TRIPATHI, 21)] = ["ot_duty"]
    fixed_shifts[(DR_KRITIKA_PRASAD, 21)] = ["morning"]
    fixed_shifts[(DR_HITESH_KUREEL, 21)] = ["evening"]
    fixed_shifts[(DR_AMIT_TRIPATHI, 21)] = ["night"]
    ...
    def my_custom_constraints(model, shift_vars):
        ...
        for dt in dates:
            ...
            day = dt.day
            max_shifts = 1
            if week == weeks.index("Sun") or day == 21:
                max_shifts = 2
            model.Add(
                sum(
                    shift_vars[(all_doctors.index(DR_AMIT_TRIPATHI), day, shift)]
                    for shift in all_shifts
                )
                <= max_shifts
            )

        # As DR_MADHURI_TRIPATHI is on leave between 16-26, evening duties should be distributed evenly between the emos
        minmax_evening_shifts = {
            DR_RASHMI_SHARMA: (3, 3),
            DR_HITESH_KUREEL: (3, 3),
            DR_KRITIKA_PRASAD: (3, 3),
            DR_MINAKSHI_MISHRA: (3, 3),
        }
        for doc, minmax_evening_shift in minmax_evening_shifts.items():
            min_evening_shift, max_evening_shift = minmax_evening_shift
            doc_idx = all_doctors.index(doc)
            sum_evening_duties = sum(
                shift_vars[(doc_idx, day, "evening")] for day in range(16, 26 + 1)
            )
            model.Add(sum_evening_duties >= min_evening_shift)
            model.Add(sum_evening_duties <= max_evening_shift)
    ...
    solution_maybe = generate_schedule(
        ...
        doctors_who_wants_do_more_shifts_per_day=[
            DR_ABHILASHA_MISHRA,
            DR_AMIT_TRIPATHI,
        ],
    )
